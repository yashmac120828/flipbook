...
async function fixPdfUrls() {
  try {
    ...
    const documents = await Document.find({
      'files.pdf.pages': { 
        $elemMatch: { 
          url: { $regex: /pg_\d+,c_limit.*c_limit/ }
        }
      }
    });
    ...
    for (const doc of documents) {
      ...
      if (doc.files && doc.files.pdf && doc.files.pdf.pages) {
        doc.files.pdf.pages = doc.files.pdf.pages.map(page => {
          if (page.url && page.url.includes('pg_')) {
            ...
            if (pageNum) {
              ...
              if (newUrl !== page.url) {
                modified = true;
                console.log(`\nDocument: ${doc._id}, Page: ${page.page}`);
        ...
        // Also update thumbnails if they have the same issue
        doc.files.pdf.pages = doc.files.pdf.pages.map(page => {
          if (page.thumbnail && page.thumbnail.includes('pg_')) {
            const match = page.thumbnail.match(/pg_(\d+)/);
            const pageNum = match ? match[1] : null;
            
            if (pageNum) {
              const baseUrl = page.thumbnail.split('/upload/')[0] + '/upload/';
              const resourcePath = page.thumbnail.split('/v1/')[1];
              const transformations = 'c_limit,h_200,q_85,w_200';
              
              const newThumbnail = `${baseUrl}${transformations}/pg_${pageNum}/v1/${resourcePath}`;
              
              if (newThumbnail !== page.thumbnail) {
                modified = true;
                console.log(`\nDocument: ${doc._id}, Page: ${page.page} (Thumbnail)`);
                console.log(`Original Thumbnail: ${page.thumbnail}`);
                console.log(`New Thumbnail: ${newThumbnail}`);
                return { ...page, thumbnail: newThumbnail };
              }
            }
          }
          return page;
